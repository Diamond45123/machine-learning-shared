...
```{r}
drosophila_data_cut <- drosophila_data |> 
  #filter(muscle %in% c(1:8, 12, 13, 16)) |>
  select(-c(ID, Species, coords))  # Drop non-numeric columns
...
drosophila_scaled <- scale(drosophila_data_cut)

dist_matrix <- dist(drosophila_scaled, method = "euclidean")
...
set.seed(42)
kmeans_model <- kmeans(drosophila_scaled, centers = 11, nstart = 25)
fviz_cluster(kmeans_model, data = drosophila_scaled, geom = "point")
